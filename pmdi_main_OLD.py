...
def runner(p_switch, # float, flooding Markov chain parameter, {0.0, 0.1}
    ...
    if impute_method in MImethods:
        print("FUCK")
        print(impute_method, K)
        imp_state_list = [true_state] * int(K)
        print(imp_state_list)
...
for i in range(start_idx*11, (start_idx*11)+11):
    ...
    for seed in range(3):
        ...
        output = runner(p_switch, p_wind_i, p_wind_j, env_missing, 
                        thetas, thetas_in, thetas_out, theta_dict, impute_method,
                        p_shuffle, num_cycles, K, epsilon, alpha, gamma, 
                        max_iters, seed, verbose)